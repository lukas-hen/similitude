...
def schemas(table_1: models.Table, table_2: models.Table) -> typing.Callable:
    ...
    names_1 = set([field for field in table_1.schema])
    names_2 = set([field for field in table_2.schema])
...
def index_match(table_1: models.Table, table_2: models.Table, indexes: list[str]) -> typing.Callable:
    table_1_names = [field.name for field in table_1.schema]
    table_2_names = [field.name for field in table_2.schema]

    for index in indexes:
        if index not in table_1_names or index not in table_2_names:
            raise ValueError(f"Error, index: '{index}' does not exist in both tables.")
